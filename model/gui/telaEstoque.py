...
class TelaEstoque(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        self.gerenciador = GerenciadorFarmacia()  # Acesso ao banco
        ...
        for i, item in enumerate(estoque):
            ...
            ctk.CTkLabel(card, text=f"Quantidade: {qtd}", font=("Arial", 12), text_color=cor).pack(anchor="w", padx=10)
            ctk.CTkLabel(card, text=f"Concentração: {item.get('concentracao', '')}", font=("Arial", 12), text_color="black").pack(anchor="w", padx=10)
            ctk.CTkLabel(card, text=f"Princípio ativo: {item.get('principio_ativo', '')}", font=("Arial", 12), text_color="black").pack(anchor="w", padx=10, pady=(0, 10))
